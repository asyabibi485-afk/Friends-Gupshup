...
# Import the module itself instead of importing individual names.
# This avoids fragile `from backend import (...)` deployment failures.
try:
    import backend
except Exception as exc:
    st.set_page_config(page_title="Friend Gup Shup", page_icon="💬")
    st.error("Backend could not be loaded.")
    st.code(f"{type(exc).__name__}: {exc}")
    st.info("Make sure app.py and backend.py are in the same GitHub folder, then redeploy.")
    st.stop()
...
config_error = backend.configuration_error()
if config_error:
...
user_id = backend.get_or_create_user()
...
if not st.session_state.profile_ready:
    ...
    if st.button("Create / Continue 💕", type="primary", use_container_width=True):
        if not name.strip():
            ...
            try:
                profile = backend.ensure_user(user_id, name)
...
try:
    profile = backend.ensure_user(user_id, st.session_state.display_name)
    st.session_state.friend_code = profile["friend_code"]
except Exception as e:
    st.error(str(e))
    st.stop()
...
with st.expander("👤 My profile & Friend Code", expanded=False):
    ...
    if st.button("Save name", use_container_width=True):
        try:
            profile = backend.ensure_user(user_id, new_name)
            st.session_state.display_name = profile["display_name"]
...
friends = backend.get_friends(user_id)
...
with st.expander("➕ Add a friend", expanded=not friends):
    ...
    if st.button("Connect friend 🤝", type="primary", use_container_width=True):
        ok, msg = backend.add_friend(user_id, code)
        if ok:
            st.success(msg)
...
messages = backend.get_messages(user_id, friend_id)
...
with st.form("send_message", clear_on_submit=True, border=False):
    ...
    if send:
        ok, msg = backend.send_message(user_id, friend_id, text)
        if not ok:
            st.error(msg)
        else:
            st.rerun()
